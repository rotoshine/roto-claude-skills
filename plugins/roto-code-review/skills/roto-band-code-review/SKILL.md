...
### Step 3.5: 프로젝트 컨텍스트 수집

에이전트를 spawn하기 **전에**, 호스트(메인 Claude)가 프로젝트 컨텍스트를 수집하여 에이전트 프롬프트에 주입한다. 이를 통해 에이전트가 프로젝트의 아키텍처, 인증 모델, 데이터 페칭 패턴 등을 이해한 상태에서 리뷰할 수 있다.

#### 수집 대상

1. **루트 CLAUDE.md** 읽기
2. **변경 파일의 상위 디렉토리 CLAUDE.md** — 변경 파일이 속한 디렉토리를 거슬러 올라가며 CLAUDE.md가 있으면 읽기 (예: `web-services/web/CLAUDE.md`)
3. **`.claude/rules/` 관련 규칙** — 디렉토리 내 파일 목록을 확인하고, 변경 내용의 키워드(auth, permission, role, fetch, cache, api, security 등)와 관련된 규칙 파일을 선별하여 읽기
4. **관련 스펙 문서** — diff에서 인증/권한/데이터 페칭 관련 변경이 감지되면 `docs/specs/` 등에 관련 스펙이 있는지 확인하여 포함

#### 수집 방법

```
1. Read: CLAUDE.md (루트)
2. Glob: **/CLAUDE.md → 변경 파일 경로와 매칭되는 것만 Read
3. Glob: .claude/rules/**
4. 파일명 + diff 키워드 매칭으로 관련 규칙 파일 2~3개 선별하여 Read
5. 수집한 내용을 "프로젝트 컨텍스트" 텍스트로 조합
```

#### 크기 제한

- 컨텍스트 총량 **3,000줄 이하** 제한
- 초과 시: CLAUDE.md + 가장 관련도 높은 규칙 파일 2~3개만 포함
- 포함하지 못한 규칙 파일은 **파일명 목록만** 전달
...
2. **프로젝트 컨텍스트**: Step 3.5에서 수집한 내용 전체. `## 프로젝트 컨텍스트` 헤더로 구분하여 삽입한다.
3. **PR 메타데이터**: title, body, author, base/head branch
4. **필터링된 Diff**: 자동 제외 파일을 제거한 diff 전문. 단, diff가 매우 길면(8,000줄+) 해당 에이전트의 우선 파일 패턴에 해당하는 파일만 전달하고, 나머지는 파일명 목록만 전달한다.
5. **변경된 파일 목록**: 파일명과 additions/deletions
6. **라인 번호 규칙**: "line은 반드시 diff가 아닌 **변경 후 파일의 실제 라인 번호**를 보고하세요. diff의 `@@ -a,b +c,d @@` 헤더에서 `+` 쪽 번호 기준입니다."
7. **컨텍스트 활용 지침**: "위 프로젝트 컨텍스트를 반드시 참고하여 리뷰하세요. 프로젝트의 기존 아키텍처/패턴/규칙에 맞지 않는 제안은 하지 마세요. 보안/성능 이슈를 지적할 때는 프로젝트의 실제 인증/권한 모델을 확인한 뒤 제안하세요."
