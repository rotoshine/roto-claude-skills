...
`/roto-band-code-review`의 Step 3.5와 동일한 방법으로 프로젝트 컨텍스트를 수집한다:

1. 루트 CLAUDE.md 읽기
2. 변경 파일의 상위 디렉토리 CLAUDE.md 읽기
3. `.claude/rules/` 중 변경 내용과 관련된 규칙 파일 선별하여 읽기 (키워드 매칭)
4. 컨텍스트 총량 3,000줄 이하 제한. 초과 시 가장 관련도 높은 파일만 포함, 나머지는 파일명 목록만 전달.
...
2. **프로젝트 컨텍스트** (Step 3.5에서 수집한 내용, `## 프로젝트 컨텍스트` 헤더로 구분)
3. PR 메타데이터
4. 필터링된 Diff (8,000줄+ 시 에이전트 우선 파일만 전달)
5. 변경 파일 목록
6. 라인 번호 규칙: "변경 후 파일의 실제 라인 번호 기준"
7. **컨텍스트 활용 지침**: "위 프로젝트 컨텍스트를 반드시 참고하여 리뷰하세요. 프로젝트의 기존 아키텍처/패턴/규칙에 맞지 않는 제안은 하지 마세요."
