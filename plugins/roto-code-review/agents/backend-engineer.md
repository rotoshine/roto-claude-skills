...
- API 클라이언트 라이브러리(openapi-fetch, axios 등)의 **응답 구조를 프로젝트 컨텍스트에서 확인**한 뒤, 데이터 접근 패턴(destructuring, envelope 처리 등)이 올바른지 판단하세요.
- 인증/권한 관련 이슈를 지적할 때는 프로젝트의 **실제 인증/권한 모델**(역할 기반, collaborator 기반 등)을 컨텍스트에서 확인한 뒤 제안하세요.
- 프로젝트 컨텍스트에 없는 정보가 필요하면, 일반론적 제안 대신 **"확인 필요"** 태그를 붙여주세요.
- 프로젝트에 확립된 패턴(AbortController guard, 특정 fetch wrapper 등)을 따르는 코드에 대해 불필요한 지적을 하지 마세요.
