...
| 에이전트 | 관점 |
|----------|------|
| UI Designer | 웹 접근성(WCAG), UX 패턴, 반응형 디자인, 시각적 일관성 |
| Senior FE Engineer | 아키텍처, 성능, 보안, 프로젝트 규칙 준수 |
| Junior FE Engineer | 가독성, 네이밍, 복잡성, 중복 패턴 |
| Backend Engineer | API 데이터 흐름, 에러 핸들링, 타입 안전성, 비동기 처리 |
| SEO Specialist | 메타데이터, URL 구조, 구조화 데이터, Core Web Vitals |
...
| 에이전트 | 관점 |
|----------|------|
| Architect | 시스템 설계, 컴포넌트 경계, 데이터 흐름, 확장성 |
| Security Engineer | 인증/인가, 데이터 보호, 입력 검증, 공격 표면 |
| Performance Engineer | 렌더링 전략, 번들 사이즈, 캐싱, Core Web Vitals |
| QA Engineer | 테스트 가능성, 엣지 케이스, 에러 시나리오, 회귀 리스크 |
| DX Engineer | API 인체공학, 코드 컨벤션, 유지보수성, 개발 워크플로우 |
